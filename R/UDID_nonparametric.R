...
#' @param SL.hpara List of Super Learner hyperparameters passed to \code{MySL}.
#' @param density Logical; return density estimates.
#' @param verbose Logical; print progress messages.
#'
#' @return A list with \code{Effect}, \code{Sensitivity}, and optionally \code{Density}.
#' @export
...
.UDID_fold <- function(Y0, Y1, A, X,
  ...
  if (type == "continuous") {
    ...
    if (hyperparameter=="fast"){
      npbw.Y0A0    <- np::npcdensbw(xdat = X[A == 0, ], ydat = Y0[A == 0], bwmethod="normal-reference")
    } else {
      npbw.Y0A0    <- np::npcdensbw(xdat = X[A == 0, ], ydat = Y0[A == 0])
    }
    kde_mat      <- matrix(stats::fitted(np::npcdens(npbw.Y0A0, exdat = eval_x_mat, eydat = eval_y_vec)), N.Test)
